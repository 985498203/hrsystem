				...
				<table class="table table-condensed table-hover" id="permtable">
					...
					<tbody>
						...
						<tr>
							...
							<td>${p.url}</td>
							<td>
								<button class="btn btn-primary btn-sm editPermButton" value="${p.permId}" >
									<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>
									编辑
								</button>
								<button class="btn btn-danger btn-sm deletePermButton" value="${p.permId}">
			...
			<div class="modal-dialog">
				...
				<form id="updatePermFrom" action="/prem">
					<div class="modal-content">
						...
						<!-- 隐藏表单：id -->
						<input type="hidden" name="permId" id="permId_update_input" value=""/>
...
<script type="text/javascript">
	...
	$(".editPermButton").click(function() {//点击修改按钮弹出新增权限资源模态框，修改权限资源
		...
		$.ajax({
			...
			success : function(JsonData) {
				if(JsonData.ret){
					$("#permId_update_input").val(JsonData.data.permId);
					$("#permName_update_input").val(JsonData.data.permName);
					$("#url_update_input").val(JsonData.data.url);
				}
			}
		});
		//
	...
	$("#perm_update_btn").click(function(){//修改按钮提交事件
		//获取表单数据
		var permId = $("#permId_update_input").val();
		var permName = $("#permName_update_input").val();
		var url = $("#url_update_input").val();
		if(null==permId || null==permName || null==url){
			alert("输入框不能为空");
		}else{
			$.ajax({
				type : 'POST',
				url : 'perm/'+permId,
				data: $('#updatePermFrom').serialize()+'&_method=PUT',//表单数据
				dataType : 'json',
				success : function(JsonData) {
					if(JsonData.ret){
						alert("修改成功");
						$('#permUpdateModal').modal('hide');//关闭模态框
						window.location.href = 'perms?pn=1';//跳转到第一页
					}else{
						alert("修改失败,请检查表单数据");
					}
				}
	...
	$(".deletePermButton").click(function() {//删除事件
		//弹出确认删除对话框
		var permName =  $(this).parents("tr").find("td:eq(1)").text();
		var permId =  $(this).parents("tr").find("input").val();
		if(confirm("确认删除：【"+permName+"】吗？")){
				//删除
			$.ajax({
				type : 'POST',
				url : 'perm/'+permId,
				data: 'id='+permId+'&_method=DELETE',//表单数据
				dataType : 'json',
				success : function(JsonData) {
					if(JsonData.ret){
						alert("删除成功");
